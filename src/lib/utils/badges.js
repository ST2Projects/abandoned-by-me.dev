...
export const badges = [
	...
	},
	{
		...
		icon: '🪦',
		description: 'More than 50% of your repos are abandoned. At least you\'re consistent.',
		check: (stats) => stats.totalCount > 0 && (stats.abandonedCount / stats.totalCount) > 0.5
	},
	{
		id: 'ghost-town',
		name: 'Ghost Town',
		icon: '👻',
		description: 'You have a repo with 0 commits after creation. Bold strategy.',
	...
	},
	{
		...
		icon: '📝',
		description: 'Multiple repos where README.md was the last (and first) commit.',
	...
	},
	{
		...
		name: 'Fork Collector',
		icon: '🍴',
		description: 'You\'ve forked 10+ repos. Contributing back is optional, apparently.',
	...
	},
	{
		...
		icon: '🌍',
		description: 'Abandoned repos in 5+ different languages. Can\'t pick a favorite?',
	...
	},
	{
		...
		icon: '🙈',
		description: 'Abandoned repo with 5+ open issues. Bless their hearts for trying.',
	...
	},
	{
		...
		icon: '🎆',
		description: 'Multiple repos created in January. New year, new abandoned projects.',
...
export function calculateBadgeStats(repositories, thresholdMonths = 6) {
	...
	const active = repositories.filter(repo => !abandoned.some(a => a.id === repo.id));
	...
	return {
		...
		readmeOnlyCount: hasEmpty ? Math.min(3, repositories.filter(r => !r.last_commit_date).length) : 0,
		weekendRepoRatio: repositories.length > 0 ? weekendRepos.length / repositories.length : 0,
...
export function getSarcasticMessage(abandonedCount, totalCount) {
	...
	if (percentage === 0) {
		...
		return "Only " + percentage.toFixed(0) + "% abandoned? You're practically a project manager.";
	} else if (percentage < 50) {
		return percentage.toFixed(0) + "% abandoned. You're right in the sweet spot of 'enthusiastic starter'.";
	} else if (percentage < 75) {
		return percentage.toFixed(0) + "% abandoned. You've mastered the art of knowing when to quit.";
	} else if (percentage < 100) {
		return percentage.toFixed(0) + "% abandoned. At this point, it's basically a hobby.";
	} else {
...
export function getUserTitle(abandonedCount) {
	if (abandonedCount >= 51) return { title: 'Tenured Professor of Abandonware', emoji: '🎓' };
	if (abandonedCount >= 36) return { title: 'CEO of Unfinished Business', emoji: '👔' };
	if (abandonedCount >= 21) return { title: 'Distinguished Quitter', emoji: '🏅' };
	if (abandonedCount >= 11) return { title: 'Professional Procrastinator', emoji: '💼' };
	if (abandonedCount >= 6) return { title: 'Serial Starter', emoji: '🔄' };
	if (abandonedCount >= 3) return { title: 'Apprentice Abandoner', emoji: '🌱' };
...
export const LANGUAGE_BADGES = {
	'JavaScript': { name: 'JavaScare', emoji: '👻', description: 'Abandoned a JavaScript project' },
	'TypeScript': { name: 'TypeSkipped', emoji: '⏭️', description: 'Abandoned a TypeScript project' },
	'Python': { name: 'Pythoff', emoji: '🐍', description: 'Abandoned a Python project' },
	'Rust': { name: 'Rusty Rust', emoji: '🦀', description: 'Abandoned a Rust project' },
	'Go': { name: 'Go Away', emoji: '🚪', description: 'Abandoned a Go project' },
	'Java': { name: 'Java-nah', emoji: '☕', description: 'Abandoned a Java project' },
	'C++': { name: 'See-Ya-Plus-Plus', emoji: '👋', description: 'Abandoned a C++ project' },
	'Ruby': { name: 'Ruby Tombstone', emoji: '💎', description: 'Abandoned a Ruby project' },
	'PHP': { name: 'PH-Porgotten', emoji: '🐘', description: 'Abandoned a PHP project' },
	'Shell': { name: 'Shell Shocked', emoji: '🐚', description: 'Abandoned a Shell project' },
	'C#': { name: 'C-Sharply Departed', emoji: '🎵', description: 'Abandoned a C# project' },
	'Swift': { name: 'Swiftly Forgotten', emoji: '🦅', description: 'Abandoned a Swift project' },
};
...
export function getLanguageBadges(repos, abandonmentThresholdMonths = 1) {
	...
	const abandoned = repos.filter(repo => {
		...
		const lastActivity = repo.last_commit_date || repo.lastCommitDate || repo.last_push_date || repo.lastPushDate;
		if (!lastActivity) return true;
		return new Date(lastActivity) < thresholdDate;
	});
...
export function getLanguageStats(repos, abandonmentThresholdMonths = 1) {
	...
	for (const repo of repos) {
		...
		if (!langMap[lang]) langMap[lang] = { language: lang, abandonedCount: 0, totalCount: 0 };
		langMap[lang].totalCount++;

		const isArchived = repo.is_archived || repo.isArchived;
		if (!isArchived) {
			const lastActivity = repo.last_commit_date || repo.lastCommitDate || repo.last_push_date || repo.lastPushDate;
			const isAbandoned = !lastActivity || new Date(lastActivity) < thresholdDate;
			if (isAbandoned) langMap[lang].abandonedCount++;
		}
	}

	const stats = Object.values(langMap)
		.map(s => ({ ...s, percentage: s.totalCount > 0 ? Math.round((s.abandonedCount / s.totalCount) * 100) : 0 }))
		.sort((a, b) => b.abandonedCount - a.abandonedCount);

	const withAbandoned = stats.filter(s => s.abandonedCount > 0);
	const mostAbandoned = withAbandoned.length > 0 ? withAbandoned[0].language : null;

	const withRepos = stats.filter(s => s.totalCount > 0);
	const safest = withRepos.length > 0
		? withRepos.reduce((min, s) => s.abandonedCount < min.abandonedCount ? s : min, withRepos[0]).language
		: null;
