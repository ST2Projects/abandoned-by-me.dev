...
describe('scanHistory', () => {
	describe('startScan', () => {
		...
		it('should set scanStartedAt', async () => {
			...
			expect(scan.scanStartedAt.getTime()).toBeGreaterThanOrEqual(before.getTime() - 1000);
		});
	});
	...
	describe('getUserScanHistory', () => {
		...
		it('should return scans ordered by createdAt descending', async () => {
			...
			await completeScan(scan1.id, { reposScanned: 5, reposAdded: 5, reposUpdated: 0 });

			// SQLite timestamp mode stores seconds, need >1s gap for ordering
			await new Promise(r => setTimeout(r, 1100));

			const scan2 = await startScan(TEST_USER_ID);
			await completeScan(scan2.id, { reposScanned: 10, reposAdded: 10, reposUpdated: 0 });
		...
		it('should respect the limit parameter', async () => {
			for (let i = 0; i < 5; i++) {
				const scan = await startScan(TEST_USER_ID);
				await completeScan(scan.id, { reposScanned: i, reposAdded: 0, reposUpdated: 0 });
			}
	...
	describe('getRunningScan', () => {
		...
		it('should return null after scan is completed', async () => {
			const scan = await startScan(TEST_USER_ID);
			await completeScan(scan.id, { reposScanned: 5, reposAdded: 5, reposUpdated: 0 });
