  ...
  try {
    ...
    const values = repositoryList.map((repo) => ({
      ...
      lastCommitDate: repo.last_commit_date
        ? new Date(repo.last_commit_date)
        : null,
 ...
 */
export async function getAbandonedRepositories(
  userId,
  ...
  try {
    ...
    thresholdDate.setMonth(
      thresholdDate.getMonth() - abandonmentThresholdMonths,
    );
  ...
  try {
    ...
    const listings = await db
      .select({
        ...
        abandonedCount:
          sql`sum(case when (${repositories.lastCommitDate} < ${thresholdDate.getTime()} or ${repositories.lastCommitDate} is null) and ${repositories.isArchived} = 0 then 1 else 0 end)`.mapWith(
            Number,
          ),
      })
      .from(userConfigs)
      ...
      .orderBy(
        sql`sum(case when (${repositories.lastCommitDate} < ${thresholdDate.getTime()} or ${repositories.lastCommitDate} is null) and ${repositories.isArchived} = 0 then 1 else 0 end) desc`,
      )
      .limit(limit)
      .offset(offset);
