...
export async function GET({ params }) {
  try {
    ...
    const publicRepositories = dashboardData.repositories.filter(
      (repo) => !repo.private,
    );
    ...
    return json({
      ...
      config: {
        abandonmentThresholdMonths:
          dashboardData.config.abandonmentThresholdMonths,
      ...
      stats: {
        ...
        lastUpdated:
          dashboardData.repositories.length > 0
            ? Math.max(
                ...dashboardData.repositories.map((r) =>
                  new Date(r.lastScannedAt).getTime(),
                ),
              )
            : null,
      },
