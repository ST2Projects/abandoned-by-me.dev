...
test('landing page shows badge preview section', async ({ page }) => {
	await page.goto('/');
	await expect(page.locator('text=Earn Badges of Dishonor')).toBeVisible();
});
...
test('login page shows privacy info cards', async ({ page }) => {
	await page.goto('/login');
	await expect(page.locator('text=No analytics')).toBeVisible();
});
...
test('dashboard config redirects to login when not authenticated', async ({ page }) => {
	await page.goto('/dashboard/config');
	await page.waitForURL(/\/login/);
	expect(page.url()).toContain('/login');
});

test('repositories API returns 401 when not authenticated', async ({ request }) => {
